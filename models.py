...
class DialogueGCNModel(nn.Module):
    ...
    def __init__(self, base_model, D_m, D_e, graph_hidden_size, n_speakers, max_seq_len,
            ...
            if not self.multi_modal:
                ...
                if 'a' in self.modals:
                    ...
                    if self.av_using_lstm:
                        self.lstm_a = nn.LSTM(input_size=hidden_a, hidden_size=D_e, num_layers=2, bidirectional=True,
                                              dropout=dropout)
                if 'v' in self.modals:
                    hidden_v = 200
        ...
        if not self.multi_modal:
            self.graph_net = GraphNetwork(2 * D_e, n_classes, n_relations, graph_hidden_size, dropout,
                                          self.no_cuda)
        else:
            if 'a' in self.modals:
    ...
    def forward(self, U, qmask, umask, seq_lengths, U_a=None, U_v=None):
        ...
        if not self.multi_modal:
            ...
            if 'v' in self.modals:
                ...
                features_v = []
            if 'l' in self.modals:
                features_l, edge_index, edge_norm, edge_type, edge_index_lengths = batch_graphify(emotions_l,
                                                                                                  qmask,
                                                                                                  seq_lengths,
                                                                                                  self.window_past,
                                                                                                  self.window_future,
